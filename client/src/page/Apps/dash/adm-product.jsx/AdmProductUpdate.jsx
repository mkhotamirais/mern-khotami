...
export default function AdmProductUpdate() {
  const { id } = useParams();
  const { singleData: data, getProductById, getProducts, updateProduct, loadUpdate, loadPage, errPage } = useProduct();
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    let result = { name, price, category, tag };
    updateProduct(id, result).then((res) => {
      if (res?.ok) {
  ...
  if (loadPage) content = <Loading />;
    ...
    content = (
      <div>
        <Title>Update Product {data?.name}</Title>
