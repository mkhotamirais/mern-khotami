...
import { ok, err, hashPass } from "../helper/utils.js";
...
export const updateUser = async (req, res) => {
  try {
    ...
    if (!email || email === "") return err(res, 400, `email required`);
    const dupUsername = await User.findOne({ username });
    if (dupUsername && username !== match.username)
      return err(res, 409, `username registered`);
    const dupEmail = await User.findOne({ email });
    if (dupEmail && email !== match.email)
      return err(res, 409, `email registered`);
    if (!validator.isEmail(email)) return err(res, 400, `email invalid`);
    if (password) {
      if (password !== confPassword)
        return err(res, 400, `confirm password wrong`);
